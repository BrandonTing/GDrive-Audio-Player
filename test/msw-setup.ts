...
export const handlers = [
  ...
  http.get('https://www.googleapis.com/drive/v3/files/:fileId', ({ params, request }) => {
    ...
    if (alt === 'media') {
      if (fileId === 'audio-1') {
        return new HttpResponse('mock audio data for song A', { headers: { 'Content-Type': 'audio/mpeg' } });
      } else if (fileId === 'audio-2') {
        return new HttpResponse('mock audio data for song B', { headers: { 'Content-Type': 'audio/wav' } });
